...
tries to seperate access mechanisms from the kernel, as we see in Twizzler.
...
These principles emerge from treating persistent data as a first class citizen.
...
how security policy for data is enforced. While the kernel doesn't manage
...
to data. Twizzler programs the MMU ( Memory Management Unit ) according to
granted access rights and then lets the hardware enforce it; more detail can be
found in section 4.2.  Now we have to define the underlying system that must be
enforced.
...
authority, holding at minimum some permissions and a unique identifier for which
...
== Our Contributions
In this thesis, I detail the fundamentals of security in the Twizzler operating
system, and discuss how I implement and refine some of the high level ideas
described in Twizzler @twizzler and an early draft of a Twizzler security paper
@twizsec. Additionally, we evaluate these systems inside kernel and user space
...
+ #link("https://github.com/twizzler-operating-system/twizzler/pull/273")[Adds creation of SigningKey / VerifyingKey object pairs.]
...
+ #link("https://github.com/twizzler-operating-system/twizzler/pull/275")[Security Contexts and Benchmarking]
  ...
  - Benchmarks for cryptographic operations inside the kernel, shown in seciton 5.
  - Userspace benchmarks of security policy creation, shown in section 5.
